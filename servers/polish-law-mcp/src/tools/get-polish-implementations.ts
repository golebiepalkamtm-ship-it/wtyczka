/**
 * get_polish_implementations — Find Polish statutes that reference a specific EU directive/regulation.
 */

import type Database from '@ansvar/mcp-sqlite';
import { generateResponseMetadata, type ToolResponse } from '../utils/metadata.js';

export interface GetPolishImplementationsInput {
  eu_document_id: string;
  primary_only?: boolean;
  in_force_only?: boolean;
}

export interface PolishImplementationResult {
  document_id: string;
...
export async function getPolishImplementations(
  db: InstanceType<typeof Database>,
  input: GetPolishImplementationsInput,
): Promise<ToolResponse<PolishImplementationResult[]>> {
  try {
  ...
  const rows = db.prepare(sql).all(...params) as PolishImplementationResult[];
