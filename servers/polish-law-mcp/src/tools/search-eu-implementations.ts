/**
 * search_eu_implementations — Search EU directives/regulations referenced by Polish legislation.
 */
...
export interface SearchEUImplementationsInput {
  ...
  year_from?: number;
  year_to?: number;
  has_polish_implementation?: boolean;
  limit?: number;
}
...
export async function searchEUImplementations(
  ...
  let sql = `
    SELECT
      ...
      ed.title,
      ed.short_name,
      COUNT(DISTINCT er.document_id) as polish_statute_count
  ...
  if (input.has_polish_implementation) {
    sql += ' HAVING polish_statute_count > 0';
  }
